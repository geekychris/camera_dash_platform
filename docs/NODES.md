...
All 39 built-in nodes, organized by category. Each entry shows its `type_id`, input/output ports, and key config options.
...
**Port types**: `FRAME` (video, drop-oldest), `DETECTIONS` (bbox sets, drop-oldest), `EVENT` (notifications, keep-all), `TRIGGER` (recording triggers, keep-all).
